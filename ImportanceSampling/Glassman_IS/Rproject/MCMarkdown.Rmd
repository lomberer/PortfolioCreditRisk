...
LossBench <- simMC(port,100000) #100k NMC simulation as a benchmark
LossMC <- simMC(port,10000) #10k NMC simulation
LossIS <- simIS(port,10000,-3) #10k IS simulation with shift=-3
...
2) Sampling distribution from ImportanceSampling is more concentrated at large loss amount.

```{r}
lossplot(LossMC) #most of the simulation is concetrated at small loss.
lossplot(LossIS) #median of loss is at large loss, generate more simulation on high loss amount, hence stabler loss measures.
